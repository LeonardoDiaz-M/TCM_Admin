...
Public Class frmLicConstruccion
    ...
    Private Sub load_Permiso()
        ...
        If id = "0" Then
            ...
            Me.BindingNavigator1.BindingSource.AddNew()
            cxn_load.Select_SQL("exec sp_DDL_Derechos 'LC',''")
            ...
            Me.BindingSource1.Current("fecha_alta") = cxn_load.arrayValores(0)
            Me.BindingSource1.Current("fecha_pago") = cxn_load.arrayValores(0)
    ...
    Private Sub load_Combos()
        cxn.Get_SQL_Combobox("exec sp_DDL_Derechos 'Construccion',''", Me.cmbTipoLicencia, "Descripcion", "clave")
    ...
    Private Function valida_datos() As Boolean
        ...
        Try
            ...
            If Not IsDate(txtFecIniObra.Value) Then
                ErrorProvider1.SetError(txtFecIniObra, "Error")
                ocurriounError += 1
                mensaje += "- " & "Ingrese una fecha válida para el inicio de obra." & "<br />"
                Me.txtFecIniObra.Focus()
            End If

            If uneDimension.Value <= 0 Then
        ...
        Try
            If valida_datos() Then
                If id <> "0" Then
                    ...
                    Me.Tbl_lic_municipalesTableAdapter1.Update(Me.DsLicencias1.tbl_lic_municipales)
                    ActivaFormulario(False)
                    cMensajes.DisplayMessage(Me, "Datos modificado correctamente!", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1)
                Else
                    'Dim fec_alta As DateTime
                    'cxn.Select_SQL("select getdate(),DATEADD(YEAR,-10,GETDATE())")
                    'fec_alta = cxn.arrayValores(0)
                    cxn.Select_SQL("exec sp_DDL_Derechos 'LC',''")
                    Me.txtNoLic.Text = cxn.arrayValores(0)

                    Tbl_lic_municipalesTableAdapter1.Insert(Me.txtNoLic.Text,
                                                               IIf(Me.cmbTipoLicencia.SelectedValue < 0, " ", Me.cmbTipoLicencia.SelectedValue), 'tip_der
                                                               Me.txtRfc.Text,
                                                               Me.txtNombre.Text,
                                                               ...
                                                               Me.txtDomicilio.Text,
                                                               Me.txtClavecatastral.Text,
                                                               " ",
                                                               Me.ucoTipoContribuyente.Value.ToString,
                                                               ...
                                                               Me.uneUltAnio.Value.ToString,
                                                               Me.mskUltMes.Text.ToString, ' ultimo mes pago
                                                               " ",
                                                               Me.uneDimension.Value.ToString,
                                                               "0",'largo
                                                               "0",'ancho                                                           
                                                               IIf(Me.chkNotificado.Checked = True, 1, 0).ToString,
                                                               IIf(Me.chkNotificado.Checked = True, Me.txtNoOficioPae.Text, ""),'numero oficio pae,
                                                               IIf(Me.chkNotificado.Checked = True, Me.txtFecPAE.Value, Nothing),'fec notif
                                                               Me.txtNoRecibo.Text,'folio rec 
                                                               Me.ucoStatus.Value,'status
                                                               Me.txtFecIniObra.Value,'fec alta
                                                               Me.mskFecemision.Value,'fec pago ,
                                                               Me.txtFecIniObra.Value,'fec ini oper,
                                                               Me.txtObservaciones.Text.Trim,
                                                               "0", 'Año inicio operaciones
                                                               Me.ucoPae.Value.ToString, 'etapa pae
                                                               IIf(Me.chkNotificado.Checked = True, Me.txtNoActos.Value, 0),
                                                               Me.txtCurp.Text,
                                                               Me.txtemail.Text,
    ...
    Private Sub txtClavecatastral_TextChanged(sender As Object, e As EventArgs) Handles txtClavecatastral.TextChanged
        If Me.txtClavecatastral.Text.Length = 16 Then
            cxn.Select_SQL("select cve_catastral,sterrprop,sconsprop from arc_predial where cve_catastral='" & Me.txtClavecatastral.Text.Trim & "'")
            If cxn.arrayValores(0) Is Nothing Then
            ...
            Else
                Me.uneTerreno.Value = cxn.arrayValores(1)
                Me.uneConstruccion.Value = cxn.arrayValores(2)
            End If
        End If
    End Sub
    Private Sub frmLicConstruccion_Activated(sender As Object, e As EventArgs) Handles Me.Activated
        txtClavecatastral.Focus()
    End Sub
    Private Sub frmLicConstruccion_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
